...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bitsliver address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BSV.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bitsliver:URI (bitsliver:XyZ...?message=example)
            {
...
static void NotifyzBSVReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBSVReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBSVReset.connect(boost::bind(NotifyzBSVReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBSVReset.disconnect(boost::bind(NotifyzBSVReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbsvTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
