...
#include "ui_zbsvcontroldialog.h"
...
std::set<std::string> ZBsvControlDialog::setSelectedMints;
std::set<CMintMeta> ZBsvControlDialog::setMints;

ZBsvControlDialog::ZBsvControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBsvControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBsvControlDialog::~ZBsvControlDialog()
{
    delete ui;
}

void ZBsvControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBsvControlDialog::updateList()
{
...
void ZBsvControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBsvControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBsv_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBsvControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBsvControlDialog::GetSelectedMints()
{
...
void ZBsvControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
