...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BSV);
    ...
    if (!settings.contains("nAnonymizeBitSliverAmount"))
        settings.setValue("nAnonymizeBitSliverAmount", 1000);

    nAnonymizeBitSliverAmount = settings.value("nAnonymizeBitSliverAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitSliverAmount"))
        SoftSetArg("-anonymizebitsliveramount", settings.value("nAnonymizeBitSliverAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitsliver.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitSliverAmount:
            return QVariant(nAnonymizeBitSliverAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitSliverAmount:
            nAnonymizeBitSliverAmount = value.toInt();
            settings.setValue("nAnonymizeBitSliverAmount", nAnonymizeBitSliverAmount);
            emit anonymizeBitSliverAmountChanged(nAnonymizeBitSliverAmount);
            break;
        case CoinControlFeatures:
