...
CzBSVTracker::CzBSVTracker(std::string strWalletFile)
{
...
CzBSVTracker::~CzBSVTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBSVTracker::Init()
{
...
bool CzBSVTracker::Archive(CMintMeta& meta)
{
...
bool CzBSVTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBSVTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBSVTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBSVTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBSVTracker::GetSerialHashes()
{
...
CAmount CzBSVTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBSVTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBSVTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBSVTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBSVTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBSVTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBSVTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBSVTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBSVTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBSVTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBSVTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBSVTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBSVTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBSVTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBSVTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBSVTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBSVTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbsv from DB\n", __func__, listDeterministicDB.size());
    }
