...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 BSV at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBSV send
enum ZerocoinSpendStatus {
    ZBSV_SPEND_OKAY = 0,                            // No error
    ZBSV_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBSV_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBSV_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBSV_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBSV_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBSV_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBSV_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBSV_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBSV_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBSV_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBSV_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBSV_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBSV_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBSV_SPENT_USED_ZBSV = 14,                      // Coin has already been spend
    ZBSV_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBSV_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZBsvBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBSVOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbsvAuto) const;
    ...
    */
    ...
    CzBSVWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBSVTracker> zbsvTracker;
    ...
    void setZWallet(CzBSVWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbsvTracker = std::unique_ptr<CzBSVTracker>(new CzBSVTracker(strWalletFile));
    }

    CzBSVWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBsvAutoBackups(bool fEnabled)
    {
    ...
    /** zBSV reset */
    boost::signals2::signal<void()> NotifyzBSVReset;
