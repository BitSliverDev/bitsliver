...
#include "config/bitsliver-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("bitsliver-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bitsliver.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bitsliverd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BSV and zBSV money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BSV/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BSV/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, bitsliver, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BSV/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BitSliver specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-bsvstake=<n>", strprintf(_("Enable or disable staking functionality for BSV inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbsvstake=<n>", strprintf(_("Enable or disable staking functionality for zBSV inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbsv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBSV minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbsvbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBSV backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebitsliveramount=<n>     " + strprintf(_("Keep N BSV anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The BitSliver Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("bitsliver-loadblk");
...
/** Sanity checks
 *  Ensure that BitSliver is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitSliver Core is shutting down."));
...
#endif
    // Make sure only a single BitSliver process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitSliver Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitSliver version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BitSliver: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //BitSliver specific: zerocoin and spork DB's
                ...
                // BitSliver: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZBSVMinted();
                        RecalculateZBSVSpent();
                    }
                    RecalculateBSVSupply(1);
                }
                ...
                // BitSliver: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitSliver Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BitSliver Core to complete") << "\n";
        ...
        zwalletMain = new CzBSVWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBSVWallet
        uiInterface.InitMessage(_("Syncing zBSV wallet..."));

        bool fEnableZBsvBackups = GetBoolArg("-backupzbsv", true);
        pwalletMain->setZBsvAutoBackups(fEnableZBsvBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbsvTracker->Init();
...
//    }
//
//    nAnonymizeBitSliverAmount = GetArg("-anonymizebitsliveramount", 0);
//    if (nAnonymizeBitSliverAmount > 999999) nAnonymizeBitSliverAmount = 999999;
//    if (nAnonymizeBitSliverAmount < 2) nAnonymizeBitSliverAmount = 2;
    ...
    LogPrintf("Anonymize BitSliver Amount %d\n", nAnonymizeBitSliverAmount);
    ...
    /* Denominations
       ...
       For example:
       1BSV+1000 == (.1BSV+100)*10
       10BSV+10000 == (1BSV+1000)*10
    */
