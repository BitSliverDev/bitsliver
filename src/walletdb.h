...
public:
    ...
    bool WriteZBSVSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBSVSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBSVSeed_deprecated(uint256& seed);
    bool EraseZBSVSeed();
    bool EraseZBSVSeed_deprecated();

    bool WriteZBSVCount(const uint32_t& nCount);
    bool ReadZBSVCount(uint32_t& nCount);
