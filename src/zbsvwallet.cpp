...
CzBSVWallet::CzBSVWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbsv seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBSVSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBSVSeed_deprecated()) {
                    LogPrintf("%s: Updated zBSV seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbsv seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbsv wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBSVWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBSVCount(nCountLastUsed);
    else if (!walletdb.ReadZBSVCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBSVWallet::Lock()
{
    seedMaster = 0;
}

void CzBSVWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBSVWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBSV(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBSVWallet::LoadMintPoolFromDB()
{
...
void CzBSVWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBSVWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBSVWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbsvTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBSVWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBSV(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbsvTracker which also adds to database
    pwalletMain->zbsvTracker->Add(dMint, true);
...
void CzBSVWallet::SeedToZBSV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBSVWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBSVWallet::GenerateDeterministicZBSV(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBSVWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBSVWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
