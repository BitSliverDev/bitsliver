...
#ifndef BITSLIVER_ZBSVWALLET_H
#define BITSLIVER_ZBSVWALLET_H
...
public:
    CzBSVWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBSV(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBSV(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BITSLIVER_ZBSVWALLET_H
